...
| Capability | Status | Notes |
| --- | --- | --- |
| Object identities | Implemented | Modern binary GUIDs and historical text-GUID joins. Stable `ObjectKind` classifications cover persisted part, assembly, component, grid, bolt, Boolean, weld, and reinforcement families while raw type/subtype values remain available as compatibility metadata. Welds additionally expose persisted workshop/site location semantics. |
...
| Relations | Implemented subset | `Stage::relations` streams stored rows and visibility state. `Stage::semantic_relations` normalizes persisted parent and evidenced stored-relation routes into parent-to-child `subelement` edges, and assembly membership into member-to-assembly `in_assembly` edges. Main members use ordinal zero; secondary and nested members use deterministic positive ordinals. Self-edges, duplicates, and dangling endpoints are omitted. Connectivity and hosted-on inference are not implemented. |
