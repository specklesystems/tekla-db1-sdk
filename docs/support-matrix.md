...
| Capability | Status | Notes |
| --- | --- | --- |
| Object identities | Implemented | Modern binary GUIDs and historical text-GUID joins. Stable `ObjectKind` classifications cover persisted part, component, grid, bolt, Boolean, weld, and reinforcement families while raw type/subtype values remain available as compatibility metadata. Welds additionally expose persisted workshop/site location semantics. |
...
| Relations | Implemented | Streams stored relation rows and visibility state. |
