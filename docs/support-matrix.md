...
| Polygon welds | Not implemented | Persisted weld identities, workshop/site classification, and the native weld properties listed above remain available, but `weldingpolygon` and historical polygon-weld rows do not yet emit `MeshView` display geometry. Logical and compound welds therefore remain semantic-only rather than receiving guessed solids. |
