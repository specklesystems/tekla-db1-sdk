...
| Capability | Status | Notes |
| --- | --- | --- |
| Object identities | Implemented | Modern binary GUIDs and historical text-GUID joins. Stable `ObjectKind` classifications cover persisted part, assembly, component, grid, bolt, Boolean, weld, reinforcement, and type-73 surface-treatment families while raw type/subtype values remain available as compatibility metadata. `ObjectRole` distinguishes independently meaningful model elements from Boolean-part and cut-plane evaluation features without removing either from raw identity batches. Welds additionally expose persisted workshop/site location semantics. |
| Native part properties | Implemented subset | Name, profile, profile parameters, material, finish, class, and form type. |
| Native surface-treatment properties | Implemented subset | Modern `surfacing`/`surfacing_attr` rows emit name, material, finish, class, numeric thickness, treatment type, and father-cut policy. Material assignments remain attached to the treatment object. |
| Native weld properties | Implemented subset | Populated modern `welding` and `welding_common_attr` rows emit integer `Tekla` properties `weldShop`, `weldAround`, `weldCompound`, and `weldLogical`; `weldIntermittentType` is emitted where that common field exists. Linked `welding_attr` rows additionally emit floating `weldSizeAbove`/`weldSizeBelow` and integer `weldTypeAbove`/`weldTypeBelow` plus `weldIntermittentAbove`/`weldIntermittentBelow`. Missing optional seam links omit only their seam properties. Common/seam attribute tables are retained as lookup indexes, while welding occurrences and their `PropertyView` values are decoded row-by-row and retained only for the current soft-budgeted output batch. Historical alternate weld-attribute layouts are not inferred. |
...
| Polygon welds | Implemented subset | Modern `weldingpolygon` groups owned by visible relation type 40001 become closed triangular fillet sweeps for native type-10 seams with positive persisted size. Polygon types 1 and 2 select the weld's above and below seam attributes; generated meshes remain attached to the weld object ID. Retained polygon rows and meshes honor the aggregate non-part geometry budget, and malformed rows fail open with object-scoped diagnostics. Logical, compound, non-fillet, zero-size, historical, and otherwise unproven welds remain semantic-only rather than receiving guessed solids. |
| Surface treatments | Implemented subset | Modern type-73 `TILE_SURFACE` rows with numeric thickness become closed polygon prisms on the surface-treatment object. Relation type 73 supplies the father Part and its persisted frame supplies the supporting-face normal; malformed, unsupported product-finish recipes, and unresolved/non-principal father faces fail open with object-scoped diagnostics. Model-space output remains the default, while opt-in local placement makes repeated treatment contours reusable. |
